...
const Paginator: React.FC<{ currentPage: number, totalPages: number, pageLimit: number, handlePagination: (page: number, limit: number) => void }> = ({ currentPage, totalPages, pageLimit, handlePagination }) => {
    ...
    return (
        <div className='flex sm:justify-end sm:items-center'>
            ...
            <div>
                ...
                <select name="limit" id="" className='border-2 p-2' value={pageLimit} onChange={(e) => { setLimit(Number(e.target.value)); handlePagination(currentPage, Number(e.target.value)) }}>
